...
# output_path = os.path.join("/ssddata/jimmy/TeleQnA/" + "sft_general_Qwen2_5_base_rank4" + "_answers_all.txt")
...
@dataclass
class TeleQNAArguments:
    teleQNA_output_path: str = field(
        default=None,
        metadata={"help": "Path to save teleQNA output"}
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, pipeline_args, teleQNA_args = parser.parse_json_file(json_file=os.path.abspath(sys.argv[1]))
    else:
        model_args, pipeline_args, teleQNA_args = parser.parse_args_into_dataclasses()
    ...
    # TeleQNA output
    output_path = teleQNA_args.teleQNA_output_path
